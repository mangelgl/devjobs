<form class="md:w-1/2 space-y-5" wire:submit.prevent="editarVacante" novalidate>
    ...
    <div>
        ...
        <x-text-input class="block mt-1 w-full" type="file" wire:model="imagenNueva" accept="image/*" />

        <div class="my-5">
            <x-input-label for="imagen" :value="__('Imagen actual')" />
            <img src="{{ asset('storage/public/vacantes') . $imagen }}" alt="{{ $titulo }}">
        </div>

        <div class="my-5">
            @if ($imagenNueva)
                Imagen nueva:
                <img src="{{ $imagenNueva->temporaryUrl() }}" alt="Preview">
            @endif
        </div>
